...
while True:
    ...
    if data.startswith("Register"):
        ...
        with open("./files/users.json", "r+") as content:
            user_data = {'name': user_info[1], 'account': user_info[2], 'DOB': user_info[3], 'password': user_info[4], 'balance': 0, 'ver': ver_code}
        ...
        # Transfer->name->account->amount->verification_code
        ...
        # Code for money depoist
        # Deposit->amount->verification_code
        print("Deposit of the money")
        logs(data)
        detial_info = data.split('->')
        ver_code = detial_info[-1]
        amount = int(detial_info[1])
        amount_added = False
        updated_data = ""
        with open("./files/users.json", "r+") as content:
        ...
        # Withdraw->amount->verification_code
