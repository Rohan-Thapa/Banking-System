...
def amount_deposit():
    ...
    if amount.isdigit():
        ...
        messagebox.showerror("Error", "The amount given for the deposit is not a integer.")
...
def amount_withdraw():
    ...
    if amount.isdigit():
        ...
        messagebox.showerror("Error", "The amount given for the deposit must be a interger.")

def amount_transfer():
    account = holder_account.get()
    name = holder_name.get()
    amount = transfer_amount.get()
    if account.isdigit() and amount.isdigit() and name != "":
        if int(amount) <= balance:
